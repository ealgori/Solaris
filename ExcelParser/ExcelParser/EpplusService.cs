...
namespace ExcelParser.Service
{
    ...
    public class EpplusService : IDisposable
    {
        ...
        public byte[] GetBytes()
        {
            ...
            app.SaveAs(stream);
            var bytes = StaticHelpers.ReadToEnd(stream);
            return bytes;
        }
        /// <summary>
        /// Вставляет картинку в драфтов на все верхние центральные колонтитулы. Нежелательно, чтоб на этом месте уже были другие колонититулы. Требует тестирования.
        /// </summary>
        /// <returns>false, если файла изображения не существует</returns>
        public bool Draft()
        {
            string imagePath = Path.Combine(Path.GetDirectoryName(new Uri(Assembly.GetExecutingAssembly().CodeBase).LocalPath), "Image", "draft.png");
            //Image logo = Image.FromFile(imagePath);
            if (File.Exists(imagePath))
            {
                app.Workbook.Worksheets.FirstOrDefault().HeaderFooter.FirstHeader.InsertPicture(new FileInfo(imagePath), PictureAlignment.Centered);
                app.Workbook.Worksheets.FirstOrDefault().HeaderFooter.OddHeader.InsertPicture(new FileInfo(imagePath), PictureAlignment.Centered);
                app.Workbook.Worksheets.FirstOrDefault().HeaderFooter.EvenHeader.InsertPicture(new FileInfo(imagePath), PictureAlignment.Centered);
                return true;
            }
            else
                return false;
        ...
        /// <summary>
        /// Вставляет таблицу в первый шит,начиная с первой ячейки.
        /// </summary>
        /// <param name="dataTable"></param>
        /// <param name="insertParameters"></param>
        /// <returns></returns>
        public string InsertTableToWorkBook(System.Data.DataTable dataTable, InsertTableParams insertParameters)
        {
            try
            {

                var sheet = app.Workbook.Worksheets[1];
        ...
        public string InsertTableToWorkSheet(string workSheetName, DataTable dataTable, InsertTableParams insertParameters)
        {
            try
            {
                var sheet = app.Workbook.Worksheets[workSheetName];
                if (sheet == null)
                {
                    sheet = app.Workbook.Worksheets.Add(workSheetName);
                }
                var startCell = sheet.Cells[1, 1];
                if (startCell != null)
                {
